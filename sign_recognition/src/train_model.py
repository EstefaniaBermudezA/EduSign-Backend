...
class SignCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x: Tensor (batch, num_features, seq_length); el eje de features es
                el de canales y el de seq_length el temporal sobre el que
                convoluciona la red.
...
def aug_drop_hand(seq):
    """Pone en cero una de las dos manos (robustez ante manos no detectadas)."""
    result = seq.copy()
...
def augment_sample(seq_raw, target_length):
    ...
    Returns:
        np.ndarray normalizado a (target_length, NUM_FEATURES).
    """
...
def load_features(features_dir):
    """Carga todas las secuencias .npy de una carpeta de clase."""
    sequences = []
...
def prepare_dataset(features_root, validation_split=VALIDATION_SPLIT, random_seed=RANDOM_SEED):
    ...
    for class_name in class_names:
        ...
        if len(seqs) > 1:
            n_train = max(1, min(n_train, len(seqs) - 1))  
...
def train(features_root, model_output_path):
    ...
    # (N, seq, feat) -> (N, feat, seq): Conv1d espera los features como canales.
...
def main():
    """Resuelve rutas del proyecto y lanza el entrenamiento sobre data/features."""
